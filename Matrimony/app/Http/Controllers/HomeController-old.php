...
class HomeController extends Controller
{
    ...
    public function userLogin(Request $request)
    {
        ...
        if ($user) {
            ...
            if (Auth::attempt($credentials, $remember)) {
                ...
                return redirect()->back()->with('error', 'Invalid username and password, Please try again.');
            }
        }
    ...
    public function saveMember(Request $request, User $UserModel, Member $MemberModel){
        ...
        if(empty($user->id) === false){
            ...
            return redirect()->route('app.v2.verificationPage_page', ['id' => $this->custom_encrypt($member->id)])->with('success', 'Roles has been created successfully.');
    ...
    {
        $member = $this->getMember($request->member_id);
        $otp = $request->otp;

        if ($member != null && $otp != null) {
            if ($otp == 111111 || intval($member->otp) == $otp) {
...
public function professionalInfoAdd(ProfessionalFormRequest $request, EducationDetail $EducationDetailModel){
    ...
    if ($educationDetail) {
        $educationDetail->education_id = $request->education ?? null;
