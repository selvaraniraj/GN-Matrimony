...
<main class="main">
   ...
   <button class="btn" onclick="history.back()">
    ...
    <option value="{{ $id }}" 
            {{ in_array($id, old('par_star', isset($search->star) ? explode(',', $search->star) : [])) ? 'selected' : '' }}>
            {{ $name }}
        </option>
                                            @endforeach
</select>
                        ...
                        <select multiple id="multiselect3" name="education[]"
                                            ...
                                            <option value="{{ $id }}" 
            {{ in_array($id, old('education', isset($search->education) ? explode(',', $search->education) : [])) ? 'selected' : '' }}>
            {{ $name }}
        </option>
                                            @endforeach
                                        </select>
    ...
    <div id="profileContainer" class="row" style="margin-bottom: 0; padding-bottom: 0;">
...
<input type="hidden" name="user_id" id="user_id" value="{{ $profile->user_id }}">
    ...
    @else
        <div class="profile-container d-inline-block position-relative">
            <img src="{{ asset('/images/user_image.png') }}" class="rounded-circle" width="150" height="150" alt="Default Image">
            <a href="#" data-bs-toggle="modal" data-bs-target="#iconModal{{ $i }}" class="lock-icon position-absolute bottom-0 rounded-circle p-2" style="top:85px;left:105px;">
    ...
    @endphp
                                    ...
                                    <div class="col-md-7">
                                        <div class="card-body">
                                            ...
                                            <p class="card-text"><span class='fw-bold'> Age:</span>  
                                    ...
                                    @endphp
                                            ...
                                            <span style="color:black">{{ $profile->cityName ?? 'Not Available' }}</span> </p>
                                            <p class="card-text "><span class='fw-bold'> Profession:</span>
                                             <span style="color:black"> {{ $profile->destination ?? 'Not Available' }} </span> </p>
                                            <p class="card-text "><span class='fw-bold'>Company:</span>
                                             <span style="color:black">{{ $profile->company_name ?? 'Not Available' }} </span> </p>
                                ...
                                <div class="row justify-content-center">
                                    ...
                                    <div class="col-md-8 justify-content-center pt-3 ms-2 position-relative">
                                        ...
                                        @else
                                            ...
                                            <div class="position-absolute bottom-0 rounded-circle p-2" style="top:60px;left:75px;">
                                    ...
                                    <div class="modal-dialog modal-dialog-centered" role="document">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-footer justify-content-center" style="border-top: none;">
                                                ...
                                                @else
                                                    <button type="button" class="btn px-4" id="profile_view{{ $i }}" onclick="profilerequest({{ $i }})">
                                                        <i class="bi bi-send"></i> Send Request
                                                    </button>
...
#add_interest{{$i}}, #interest{{$i}}, #not_interest{{$i}} {
    margin-top: 20px;
}
