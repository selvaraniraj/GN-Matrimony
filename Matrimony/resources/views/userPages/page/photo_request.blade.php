...
<input type="hidden" name="id" id="profile_id{{ $index }}" value="{{ $member->id }}">
                                          ...
                                          <div class="wrapper-max" style="border:3px solid #e5e5e5;">
                                                <div class="card">
                                                      ...
                                                      <div class="row">
                                                            ...
                                                            onclick="view_person(event, this)" data-profile-id="{{ $log->id }}"  data-name="{{ $log->name }}" 
    ...
    @if (in_array($profileId, $upprovePhoto))
        @if(!empty($log->file_path))
            <div class="profile-container d-inline-block position-relative">
                <img src="{{ asset($log->file_path) }}" class="rounded-circle" 
    ...
    @else
        <div class="profile-container d-inline-block position-relative">
            <img src="{{ asset('/images/user_image.png') }}" class="rounded-circle" width="150" height="150" alt="Default Image">
            <a href="#" data-bs-toggle="modal" data-bs-target="#iconModal{{ $i }}" class="lock-icon position-absolute bottom-0 rounded-circle" style="top:90px;left:115px;">
                <i class="bi bi-person-fill-lock fs-4 text-dark"></i>
            </a>
        </div>
    @endif

    <h5 class="card-title mt-2" style="font-weight: bold;color:#8C0000;">{{ $log->name ?? 'Unknown' }}</h5>
